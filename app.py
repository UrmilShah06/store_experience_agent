    ...
    for tool in registry["tools"]:
        with st.expander(f"Tool: {tool['name']}"):
            ...
            for param, details in tool["parameters"].items():
                ...
                # FIX: safe .get() handles params without description key
                desc_text = details.get("description", details.get("type",""))
                st.markdown(f"- `{param}` ({req}): {desc_text}")
            st.markdown(f"**Returns:** {tool['returns']}")

    st.markdown("---")
    st.markdown("### Test MCP Tools Live")
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("**get_store_layout**")
...
<p>Store data lives in silos — IoT behavioural patterns, weekly sales performance, gondola allocation, and the promotional calendar are never synthesised together. A VM manager reads the planogram. A store manager reads the sales report. Nobody sees that customers are spending 4 minutes in a category but converting at only 18% — and that the top-converting brand is on the bottom shelf. This agent connects all four data sources automatically and produces a monthly gondola action plan.</p>
